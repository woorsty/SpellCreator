...
function drawCard(
  ...
  if (card.index == 1) {
    ...
    if (card.komponenten.gestik) {
      addHorizontalIcon(
        doc,
        ICON_PATH + "Intimidation_Icon.png",
        imagePositions[componentCounter][counter++],
        cursorY,
      );
    }
...
function printPropertyLine(
  ...
  if (card.zeitaufwand === "Aktion") {
    addHorizontalIcon(doc, ICON_PATH + "34px-Action_Icon.png", coursorX, y);
    addHorizontalText(doc, "Aktion", coursorX + 4, y);
  } else if (card.zeitaufwand === "Bonusaktion") {
    addHorizontalIcon(
      doc,
      ICON_PATH + "34px-Bonus_Action_Icon.png",
      coursorX,
      y,
    );
    addHorizontalText(doc, "Bonusaktion", coursorX + 4, y);
  } else if (card.zeitaufwand === "Reaktion") {
    addHorizontalIcon(doc, ICON_PATH + "23px-Reaction_Icon.png", coursorX, y);
  ...
  if (card.ritual) {
    addHorizontalIcon(
      doc,
      ICON_PATH + "Ritual_Spell_Icon.png",
      x + width / 2 - 11,
      y,
    );
  ...
  if (card.konzentration) {
    addHorizontalIcon(
      doc,
      ICON_PATH + "25px-Concentration_Icon.png",
      x + width - 25,
      y,
    );
...
function addHorizontalIcon(doc: jsPDF, iconPath: string, x: number, y: number) {
  const icon = fs.readFileSync(path.resolve(iconPath));
