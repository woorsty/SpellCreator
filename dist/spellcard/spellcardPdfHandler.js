...
function generateCardPDF(spells, res) {
...
function drawCard(doc, spell, x, y, w, h) {
...
function drawCardMultiPage(doc, spell, x, y, w, h) {
    const padding = 6;
    const innerX = x + padding;
    const innerY = y + padding;
    const innerW = w - 2 * padding;
    ...
    const clean = (s) => s
        ...
        .trim();
    const fullText = clean(spell.Text) +
        (spell.HöhereLevel
